import groupRepository from '../../repositories/message/group.repository.js'
...
class GroupController {
	...
	async getAll(req, res) {
		...
		try {
			const groups = await groupRepository.getAll(fk_store_id)
	...
	async update(req, res) {
		...
		const existingGroup = await groupRepository.getById(id)
		if (!existingGroup) {
			return res
				.status(404)
		...
		try {
			const updateGroup = await groupRepository.update(id, data)
			// Retorno da API
			res.status(200).json({
				success: true,
